...
//调用方式
$('.J-timePickerRange-1').timePickerRange({
    //是否是范围选择
    isRange: true,
...
$(function() {
    ...
    var templateParent = '<div class="timePickerRange-wrap">' +
                           '{{item}}'+
                        '</div>';
    ...
    function TimePickerRange(ele, options) {
        if (!ele.length) {
            return false;
        }
        this.target = ele;
    ...
    TimePickerRange.prototype = {
        ...
        event: function() {
            ...
            $body.off('click', '.timePicker-next,.timePicker-prev').on('click', '.timePicker-next,.timePicker-prev', function(event) {
                var $this = $(this);
                var _this = $('.timePickerRange-wrap').data('object');
            ...
            $body.off('click', '.timePicker-year').on('click', '.timePicker-year', function(event) {
                var _this = $('.timePickerRange-wrap').data('object');
                _this.updateRenderPicker();
                $('.timePickerRange-wrap .timePicker-time-main').addClass('hide');
            ...
            $body.off('click', '.timePicker-day').on('click', '.timePicker-day', function(event) {
                var _this = $('.timePickerRange-wrap').data('object');
                $('.timePickerRange-wrap .timePicker-time-main').removeClass('hide');
                showDayMain.call(_this, 'day');
            });
            ...
            this.target.on('click', '[role="timePicker"]', function(event) {
                ...
                var length = _this.target.find('.timePickerRange-wrap').length;
                //1.如果不是范围则不处理第二个输入框。
                //2.不处理大于2的输入框
                ...
                if (!$('.timePickerRange-wrap').length) {
                    _this.showPicker($(this));
                }
            });

            //阻止冒泡
            $body.off('click', '.timePickerRange-wrap').on('click', '.timePickerRange-wrap', function(event) {
                event.stopPropagation();
            });
        ...
        showPicker: function(target) {
            ...
            if (formatType(this.options.format) === 1) {
                $('.timePickerRange-wrap .timePicker-time-main').remove();
            }
            if (formatType(this.options.format) === 2) {
                $('.timePickerRange-wrap .timePicker-second-main').remove();
            }
            ...
            //存储实例化对象到时间显示元素timePickerRange-wrap
            $('.timePickerRange-wrap').data('object', this.target.data('object'));
        ...
        hidePicker: function() {
            ...
            $body.find('.timePickerRange-wrap').remove();
            //隐藏后回调
        ...
        updateRenderPicker: function() {
            ...
            var html = template.replace('{{dayItem}}', this.renderDate(result.index))
                ...
                .replace('{{year}}', date.year)
                .replace('{{day}}', date.day);
            $('.timePickerRange-wrap').html(html);
            initActiveItem.call(this);
            if (formatType(this.options.format) === 1) {
                $('.timePickerRange-wrap .timePicker-time-main').remove();
            }
    ...
    function setNavDisable(time) {
        if ((time.year > this.maxDate.year) || (time.year == this.maxDate.year && time.month >= this.maxDate.month)) {
            $('.timePickerRange-wrap .timePicker-next').addClass('timePicker-disable');
            return;
        }
        if ((time.year < this.minDate.year) || (time.year == this.minDate.year && time.month <= this.minDate.month)) {
            $('.timePickerRange-wrap .timePicker-prev').addClass('timePicker-disable');
            return;
        }
        $('.timePickerRange-wrap .timePicker-next').removeClass('timePicker-disable');
        $('.timePickerRange-wrap .timePicker-prev').removeClass('timePicker-disable');
    }
    ...
    function showDayMain(classType) {
        ...
        var $time = $('.timePickerRange-wrap .timePicker-time-main');
    ...
    $.fn.timePickerRange = function (options) {
        return this.each(function () {
            new TimePickerRange($(this), options);
        });
    };
}());

$(function(){
    $('.J-timePickerRange-3').timePickerRange({
        //是否是范围选择
        isRange: true,
        minDate:'1980-10-22',
        maxDate:'1993-10-29'
    });
    $('.J-timePickerRange').timePickerRange({
        //是否是范围选择
        isRange: true
    });
    ...
    $('.J-timePickerRange-2').timePickerRange({
        isRange: false,
        format:'YYYY-MM-DD'
    });
});
